...
def calibrate_punch():
    ...
    print(f"  Raw accelerations: {[f'{abs(row[0]):.1f}' for row in data if abs(row[1]) > 9.0][:5]}")
    print(f"  Jerk forces: {[f'{f:.1f}' for f in jerk_forces[:5]]}")
    print(f"  Calculated threshold: {threshold:.2f} (vs old method: ~{max([abs(row[0]) for row in data if abs(row[1]) > 9.0]):.1f})")
    return threshold
...
def calibrate_jump():
    ...
    print(f"  Raw accelerations: {[f'{abs(row[0]):.1f}' for row in data if abs(row[0]) > 9.0][:5]}")
    print(f"  Jerk forces: {[f'{f:.1f}' for f in jerk_forces[:5]]}")
    print(f"  Calculated threshold: {threshold:.2f} (vs old method: ~{max([abs(row[0]) for row in data if abs(row[0]) > 9.0]):.1f})")
    return threshold
